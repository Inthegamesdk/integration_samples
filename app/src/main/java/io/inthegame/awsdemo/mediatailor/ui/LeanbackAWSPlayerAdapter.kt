 ...
 */
package io.inthegame.awsdemo.mediatailor.ui
...
class LeanbackAWSPlayerAdapter(
    private val context: Context,
    private val player: Player,
    private val updatePeriodMs: Int
) : PlayerAdapter(), Runnable {
    private val handler: Handler = Util.createHandlerForCurrentOrMainLooper()
    ...
    override fun getDuration(): Long {
        val durationMs = if (player.isCurrentMediaItemLive) -1 else player.duration
        return if (durationMs == C.TIME_UNSET) -1 else durationMs
    }

    override fun getCurrentPosition(): Long {
        return if (player.playbackState == Player.STATE_IDLE) -1 else if (player.isCurrentMediaItemLive) player.periodPositionMs
            ?: player.currentPosition else player.currentPosition
    }
    ...
    private inner class PlayerListener : Player.Listener, SurfaceHolder.Callback {
        ...
        override fun onTimelineChanged(
            ...
            val callback = callback
            callback.onDurationChanged(this@LeanbackAWSPlayerAdapter)
            callback.onCurrentPositionChanged(this@LeanbackAWSPlayerAdapter)
            callback.onBufferedPositionChanged(this@LeanbackAWSPlayerAdapter)
        }
